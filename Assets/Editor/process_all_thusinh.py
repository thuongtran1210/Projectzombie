...
ACTIONS = {
    "Idle": "media_1787737738645.png",
    "Run": "media_1787737754701.png",
    "Attack": "media_1787737764310.png",
    "Dash": "media_1787737801103.png",
    "Dead": "media_1787737860431.png"
}
...
print(f"ThuSinh Master Idle Raw Height: {master_idle_raw_h:.2f} px")

# Khóa cứng hệ số scale toàn cục (GLOBAL_SCALE) cho Thư Sinh = 96px chuẩn
GLOBAL_SCALE = 96.0 / master_idle_raw_h
print(f"LOCKED GLOBAL_SCALE FOR THUSINH = {GLOBAL_SCALE:.4f}")

def process_action_thusinh(action_name, filename):
    src_path = os.path.join(MEDIA_DIR, filename)
    dst_path = os.path.join(OUTPUT_DIR, f"ThuSinh-{action_name}.png")
    ...
    for idx, sl in enumerate(valid_slices[:6]):
        ...
        is_skin = (r > 165) & (g > 105) # da mặt, tay, chân
        is_blue_robe = (b > 120) & (g > 100) # áo lam ngũ thân
        is_scroll = (r > 140) & (g > 110) & (b > 70) # cuộn sách tre
    ...
    strip.save(dst_path)
    print(f"Master Clean: ThuSinh-{action_name}.png ({len(frames)} frames) done!")

for act, file_name in ACTIONS.items():
    process_action_thusinh(act, file_name)
