...
# GLOBAL_SCALE chuẩn cho Ma Đói Nợ = 96px
GLOBAL_SCALE = 96.0 / master_idle_raw_h
print(f"Madoino Master Idle Raw Height: {master_idle_raw_h:.2f} px, LOCKED GLOBAL_SCALE = {GLOBAL_SCALE:.4f}")

def process_action_madoino(action_name, filename):
    ...
    if action_name == "Dead":
        # Grid 2x3 cho Dead (6 frames) dựa theo tỉ lệ ảnh 1024x506
        # Hàng 1: y 0..253, Hàng 2: y 253..506
        # Cột: 0..341, 341..682, 682..1024
        half_h = h // 2
        ...
        for idx, box in enumerate(boxes):
            ...
            # Ngưỡng nhạy để giữ lại các đồng tiền vàng và khói xanh
            threshold = 20.0 if idx >= 2 else 30.0
        ...
        # Idle, Run, Attack (Grid 2x3 tiêu chuẩn)
        ...
        for idx, sl in enumerate(valid_slices[:6]):
            ...
            is_mint = (g > 130) & (b > 120) # da ma xanh ngọc
            is_sack = (r > 120) & (g > 90) & (b < 80) # bao tải tiền
            is_coin = (r > 150) & (g > 120) & (b < 90) # tiền xu vàng
            is_safe = is_dark | is_mint | is_sack | is_coin
            alpha[is_bottom & (dist_bg < 65.0) & (sat < 0.16) & (~is_safe)] = 0
            
            # Xóa sparkle ở góc dưới bên phải frame 6
            if idx == 5:
    ...
    strip.save(dst_path)
    print(f"Master Clean: Madoino-{action_name}.png ({len(frames)} frames) done!")

for act, file_name in ACTIONS.items():
    process_action_madoino(act, file_name)
