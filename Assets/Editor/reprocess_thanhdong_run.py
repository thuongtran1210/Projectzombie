...
SRC_IMAGE = r"C:\Users\thuon\.gemini\antigravity-ide\brain\b3d411d8-f012-497c-ace6-450430cca8a8\.user_uploaded\media_1787738715442.png"
OUTPUT_DIR = r"C:\Users\thuon\Unity\Projectzombie\Assets\Art\ThanhDong"
PNG_PATH = os.path.join(OUTPUT_DIR, "ThanhDong-Run.png")
META_PATH = os.path.join(OUTPUT_DIR, "ThanhDong-Run.png.meta")
ANIM_PATH = os.path.join(OUTPUT_DIR, "Run.anim")

GLOBAL_SCALE = 0.4192

# 8 ô Grid 2x4 chuẩn cho ảnh kích thước 1024x572
...
for idx in range(8):
    ...
    is_red = (r > 135) & (g < 65)
    is_green = (g > 80) & (r < 90)
    is_white_silk = (lum > 180) & (sat < 0.10) # dải lụa trắng
    is_safe = is_dark | is_skin | is_red | is_green | is_white_silk
    alpha[is_bottom & (dist_bg < 65.0) & (sat < 0.16) & (~is_safe)] = 0
    
    # 3. Xóa sparkle ở góc dưới bên phải frame 8
    if idx == 7:
...
# Ghi file PNG 8 frames
strip = Image.new('RGBA', (len(frames) * 128, 128), (0, 0, 0, 0))
for i, f in enumerate(frames):
    strip.paste(f, (i * 128, 0), f)
strip.save(PNG_PATH)
print(f"Baking ThanhDong-Run.png completed with {len(frames)} frames!")

guid = "4100000000000000a0b77823fca88d02"
fps = 14
is_loop = 1
...
for i in range(len(frames)):
    s_name = f"ThanhDong-Run_{i}"
    internal_id = 4100000000 + (1 * 100000) + (i * 12345)
...
print("Updated ThanhDong-Run.png, .meta and Run.anim successfully!")
