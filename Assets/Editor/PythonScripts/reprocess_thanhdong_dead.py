...
SRC_IMAGE = r"C:\Users\thuon\.gemini\antigravity-ide\brain\b3d411d8-f012-497c-ace6-450430cca8a8\.user_uploaded\media_1787736387081.png"
OUTPUT_DIR = r"C:\Users\thuon\Unity\Projectzombie\Assets\Art\ThanhDong"
PNG_PATH = os.path.join(OUTPUT_DIR, "ThanhDong-Dead.png")
META_PATH = os.path.join(OUTPUT_DIR, "ThanhDong-Dead.png.meta")
ANIM_PATH = os.path.join(OUTPUT_DIR, "Dead.anim")
...
# Định nghĩa tọa độ 6 keyframes chuẩn cho animation Dead:
# 1. Bị đánh trúng (Hàng 1, Cột 1)
# 2. Ngã về sau buông rơi quạt (Hàng 1, Cột 2)
# 3. Tiếp đất đập lưng (Hàng 1, Cột 3)
# 4. Trượt ngã ngang (Hàng 1, Cột 4)
# 5. Nằm yên bất động (Hàng 2, Cột 1)
# 6. Bốc hơi hóa thành đàn hoa sen linh hồn (Hàng 2, Cột 2->4 gom thành 1 frame biến mất hoành tráng)

BOXES = [
    (0, 0, int(w * 0.25), mid_y),
    (int(w * 0.25), 0, int(w * 0.50), mid_y),
    (int(w * 0.50), 0, int(w * 0.75), mid_y),
    (int(w * 0.75), 0, w, mid_y),
    (0, mid_y, int(w * 0.30), h),
    (int(w * 0.45), mid_y, w, h) # Cụm hoa sen linh hồn bốc lên hoàn chỉnh
]
...
for idx, box in enumerate(BOXES):
    ...
    top_lbls = [l_i for l_i, sz in enumerate(sizes) if sz > 150]
    ...
    is_red = (r > 135) & (g < 65)
    is_green = (g > 80) & (r < 90)
    is_fan = (r > 150) & (g > 120) & (b > 90)
    is_safe = is_dark | is_skin | is_red | is_green | is_fan
    alpha[is_bottom & (dist_bg < 65.0) & (sat < 0.16) & (~is_safe)] = 0
    
    # 3. Xóa sparkle
    if idx == 5:
        is_sparkle = (lum > 175) & (sat < 0.12)
        alpha[is_sparkle] = 0
...
# Ghi file PNG 6 frames chuẩn khít
strip = Image.new('RGBA', (len(frames) * 128, 128), (0, 0, 0, 0))
for i, f in enumerate(frames):
    strip.paste(f, (i * 128, 0), f)
strip.save(PNG_PATH)
print(f"Baking ThanhDong-Dead.png completed with {len(frames)} frames!")

guid = "4100000000000000a0b77823fca88d05"
fps = 8
is_loop = 0
...
for i in range(len(frames)):
    s_name = f"ThanhDong-Dead_{i}"
    internal_id = 4100000000 + (4 * 100000) + (i * 12345)
...
print("Updated ThanhDong-Dead.png, .meta and Dead.anim successfully!")
