...
def clean_checkerboard_dash_master():
    ...
    for idx, box in enumerate(boxes):
        ...
        # Riêng frame 4: Chỉ xóa sparkle (ngôi sao xám trắng nhỏ cạnh bàn chân phải)
        if idx == 3:
            # Ngôi sao 4 cánh là đốm nhỏ xám nhạt không màu (sat < 0.10) và lum > 140 ở góc dưới chân phải
            sparkle_spot = (np.arange(h_sub)[:, None] > int(h_sub * 0.82)) & (np.arange(w_sub)[None, :] > int(w_sub * 0.60)) & (np.arange(w_sub)[None, :] < int(w_sub * 0.76))
            is_sparkle = sparkle_spot & (sat < 0.10) & (lum > 140)
            alpha[is_sparkle] = 0
    ...
    strip.save(dst_path)
    print("DASH STRIP 100% PERFECT & FLAWLESS!")
